...
class EcoApproval(models.Model):
    ...
    awaiting_my_validation=fields.Boolean('Awaiting my validation',compute_sudo=True,compute='_compute_status',search='_search_awaiting_my_validation')
    awaiting_validation=fields.Boolean('Awaiting validation',compute_sudo=True,compute='_compute_status',search='_search_awaiting_validation')
    ...
    is_pending=fields.Boolean('Is pending',compute_sudo=True,compute='_compute_status',store=True,readonly=True)
    is_approved=fields.Boolean("Is approved",compute_sudo=True,compute='_compute_status',store=True,readonly=True)
    is_rejected=fields.Boolean("Is rejected",compute_sudo=True,compute='_compute_status',store=True,readonly=True)
    ...
    def _compute_status(self):
        for record in self:
            ...
            record.awaiting_validation=record.is_pending 
    ...
    def _search_awaiting_validation(self, operator, value):
        recs = self.search([]).filtered(lambda x : x.status in ['none'] )
        if recs:
            return [('id', 'in', [x.id for x in recs])]
        else:
            return [('id', '=', False)]