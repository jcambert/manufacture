...
class ProductTemplate(models.Model):
    ...
    def _compute_eco_count(self):
        for product in self:
            ...
            product.eco_count_blocking =len( not_dones.filtered(lambda x: (x.sale_ok and not  x.can_manufacture) or (x.purchase_ok and not x.can_purchase)))
    ...
    def recalcul_eco_count(self):
        ...
        eco_count_blocking =len( not_dones.filtered(lambda x: (x.sale_ok and not  x.can_manufacture) or (x.purchase_ok and not x.can_purchase)))
